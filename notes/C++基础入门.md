...
案例描述：从1开始数到数字100， 如果数字个位含有7，或者数字十位含有7，或者该数字是7的倍数，我们打印敲桌子，其余数字bu输出。
...
int main() {
	...
	const int * p1 = &a; //固定的是地址上的值，10
	p1 = &b; //正确
	//*p1 = 100;  报错
	//指针指向的是地址，const后面的是不可修改的值
    //const修饰的是指针时，意思是指针的值固定也就是地址

	//const修饰	是常量，指针指向不可以改，指针指向的值可以更改
	int * const p2 = &a;//固定的是地址的值,000000D3F959F9E4
	//p2 = &b; //错误
	*p2 = 100; //正确
